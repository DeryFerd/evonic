...
def test_preserved_archives_when_over_max_cap():
    ...
    # 3 preserved: A1(2000), A2(3000), A3(4000) → oldest A1 must archive
    result = store.tick_lifecycle(ms.cmp, now_ts=5000)
    ...
    assert ms.cmp['paths']['A4']['status'] == 'active'
...
def test_reactivation_resets_the_lifecycle_clock():
    ...
    # 3 preserved: A1(2000), A2(3000), A3(4000) → A1 archived
    store.tick_lifecycle(ms.cmp, now_ts=5000)
    assert ms.cmp['paths']['A1']['status'] == 'archived'

    store.switch_to(ms.cmp, ms, 'A1', now_ts=5100)           # reactivate
    p1 = ms.cmp['paths']['A1']
    assert p1['status'] == 'active' and p1['state_since'] == 5100

    store.switch_to(ms.cmp, ms, 'A4', now_ts=5200)           # suspend again
    assert p1['status'] == 'preserved'                       # NOT archived (fresh clock)
    # A1 state_since=5200, A2=3000, A3=4000 → A2 is now oldest → archived
    result = store.tick_lifecycle(ms.cmp, now_ts=5300)
...
def test_active_lineage_is_exempt_from_archiving():
    ...
    store.switch_to(ms.cmp, ms, 'B1', now_ts=5100)
    # B1 active, protected={A1}. Preserved: A2(4000), A3(5000), A4(5100)
    # Unprotected = 3 > 2 → A2 (oldest) archived; A1 exempt
    result = store.tick_lifecycle(ms.cmp, now_ts=5200)
...
def test_legacy_dormant_status_normalizes():
    ...
    # 2 preserved: A1(2000), A2(3000) — exactly at cap, no archiving
    p1 = ms.cmp['paths']['A1']
    ...
    result = store.tick_lifecycle(ms.cmp, now_ts=3100)
